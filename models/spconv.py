...
class ResUNet(ME.MinkowskiNetwork):
    ...
    def __init__(self, network_config,
                 in_channels=3, out_channels=32, n_classes=14,
                 normalize_feature=None, conv1_kernel_size=None, D=3):
        super().__init__(D)
        ...
        self.classifier = ME.MinkowskiConvolution(
            in_channels=tr_channels[1], out_channels=n_classes,
            kernel_size=1, stride=1, dilation=1, bias=True, dimension=D)
    ...
    def forward(self, x):
        ...
        if self.normalize_feature:
            return ME.SparseTensor(
                feat.F / torch.norm(feat.F, p=2, dim=1, keepdim=True),
                coordinate_map_key=feat.coordinate_map_key,
                coordinate_manager=feat.coordinate_manager), cls
        else:
            return feat, cls
