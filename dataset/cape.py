...
class CAPEDataset(Dataset):
    ...
    def __getitem__(self, index) -> dict:
        ...
        pc1 = np.ascontiguousarray(pc1[:, reaxis])
        pc2 = data['pcs'][j].astype(np.float32)
        pc2 = np.ascontiguousarray(pc2[:, reaxis])
        ret["pcs"] = [pc1, pc2]
        
        # Load labels
        ret["labels"] = [None, None]

        # Load flows
        flow12 = data['flows'][i, j].astype(np.float32)
        flow21 = data['flows'][j, i].astype(np.float32)
        ret["flows"] = [flow12[:, reaxis], flow21[:, reaxis]]
